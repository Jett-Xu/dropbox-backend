...
專案啟動後（**注意：此專案包含開發測試用的自動重置機制，每次執行重啟時，都會自動清除舊資料庫並重新載入 `seed-data.json` 裡的預設資料，確保每次測試的資料純淨度**），可以透過瀏覽器訪問 Scalar API 文件介面查看和測試所有 API 端點 (通常路徑為 `http://localhost:<port>/scalar/v1`)。
